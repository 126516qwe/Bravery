...
public class Inventory_Storage : Inventory_Base
{
    ...
    private bool HasEnoughMaterials(Inventory_Item itemToCraft)
    {
        ...
        {
            if (GetAvailaelAmountOf(requiredMaterial.itemData) < requiredMaterial.stackSize)
                return false;
        }
        return true;
    }

    public int GetAvailaelAmountOf(Item_DataSO requiredItem)
    {
        int amount = 0;
    ...
    {
        var stackatbleItem = StackableInStash(itemToAdd);

        if (stackatbleItem != null)
            stackatbleItem.AddStack();
        else
        {
