...
public class UI_TreeConnectHandler : MonoBehaviour
{
    ...
    {
        List<UI_TreeNode> childrenToRenturn = new List<UI_TreeNode>();

        foreach (var node in connectionDetails) 
        {
            if (node.childNode != null)
                childrenToRenturn.Add(node.childNode.GetComponent<UI_TreeNode>());
        }

        return childrenToRenturn.ToArray(); 
    }
    ...
    private void OnValidate()
    {
        ...
        {
            Debug.Log("Amount of details should be same as" + gameObject.name);
            return;
        }
