...
public class UI_TreeConnection : MonoBehaviour
{
    ...
    {
        bool shouldBeAction = direction!=NodeDirectionType.None;
        float finalLength = shouldBeAction? length : 0; 
        float angle = GetDirectionAngle(direction);

        rotationPoint.localRotation = Quaternion.Euler(0,0,angle+offset);//Ðý×ª
        connectionLength.sizeDelta= new Vector2(finalLength,connectionLength.sizeDelta.y);//³¤¶È
    }
